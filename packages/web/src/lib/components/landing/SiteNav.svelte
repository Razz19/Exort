...
<nav
  class={`sticky top-0 z-30 transition-[background-color,box-shadow,backdrop-filter] duration-200 ease-out ${
    isNavScrolled
      ? "bg-[rgba(40,40,40,0.92)] shadow-nav backdrop-blur-[30px]"
...
>
  {#if isMobileMenuOpen}
    <button
      type="button"
      class="fixed inset-x-0 bottom-0 top-[5.25rem] z-0 bg-[rgba(29,32,33,0.24)] backdrop-blur-md md:hidden"
  ...
  <div
    class="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8"
  >
    ...
    <div class="flex items-center gap-2 sm:gap-4">
      <a
        ...
        class="hidden md:inline-flex items-center justify-center text-gruvbox-text transition-colors duration-200 hover:text-gruvbox-orange focus-visible:text-gruvbox-orange focus-visible:outline-none motion-reduce:transition-none"
      >
      ...
      <a
        ...
        class="hidden md:inline-flex items-center justify-center text-gruvbox-text transition-colors duration-200 hover:text-gruvbox-blue focus-visible:text-gruvbox-blue focus-visible:outline-none motion-reduce:transition-none"
      >
  ...
  <div
    id="mobile-nav-menu"
    class={`absolute inset-x-0 top-full z-20 px-4 transition-[opacity,transform] duration-200 ease-out md:hidden motion-reduce:transition-none ${
      isMobileMenuOpen
  ...
  >
    <div
      class="mx-auto flex max-w-7xl flex-col gap-1 border-t-0 bg-gruvbox-bg px-4 py-4 shadow-soft"
    >
      {#each navItems as item}
        <a
          href={item.href}
